...
const Inventory: React.FC = () => {
  ...
  const lowStockProducts = filteredProducts.filter(product => product.stock_quantity < 100);
  const regularProducts = filteredProducts.filter(product => product.stock_quantity >= 100);
  ...
  const ProductRow: React.FC<{ product: Product; isLowStock?: boolean }> = ({ product, isLowStock = false }) => (
    <motion.tr
            ...
            <div className="bg-white rounded-xl shadow-lg overflow-hidden">
              <div className="bg-red-50 border-b border-red-200 px-6 py-4">
                ...
                <p className="text-red-700 text-sm mt-1">
                  Các sản phẩm có số lượng tồn kho dưới 100 đơn vị
                </p>
              </div>
                ...
                <table className="w-full">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {lowStockProducts.map((product) => (
                      <ProductRow key={product.id} product={product} isLowStock={true} />
                    ))}
                  </tbody>
                </table>
              </div>
