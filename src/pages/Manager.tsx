...
const Manager: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const aggregateDailyRevenue = () => {
      ...
      // Create chart data for the last 7 days (including days with zero revenue)
  ...
  const fetchOrders = async () => {
    try {
      ...
      // Fetch orders for the last 30 days to ensure enough data for stats and chart
      const now = new Date();
      const startDate = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000).toISOString();
      
      const response = await fetch(
        `${supabaseUrl}/rest/v1/orders?select=*,order_items(*,products(name)),users(full_name,email,phone)&created_at=gte.${startDate}&order=created_at.desc&limit=50`,
        {
          headers: {
            'apikey': supabaseKey,
        ...
        <div className="space-y-8">
          ...
          {chartData.length === 0 ? (
            <div className="bg-white rounded-xl shadow-lg p-6 text-center">
              <p className="text-gray-600">No revenue data available for the last 7 days.</p>
            </div>
          ) : (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
