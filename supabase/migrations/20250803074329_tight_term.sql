/*
  # Fix Inventory table foreign key relationship

  1. Drop and recreate inventory table with proper foreign key
    - Ensure proper foreign key constraint to products table
    - Maintain all existing functionality and constraints

  2. Re-populate sample data
    - Add inventory records for all branches and products
*/

-- Drop existing inventory table if it exists
DROP TABLE IF EXISTS inventory CASCADE;

-- Create inventory table with proper foreign key relationship
...
CREATE INDEX idx_inventory_product_branch ON inventory(product_id, branch);
