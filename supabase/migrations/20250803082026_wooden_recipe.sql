/*
  # Add inventory data and inventory_id column to products

  1. New Data
    - Insert inventory records for all existing products across different branches
    - Add inventory_id column to products table
    - Update products with corresponding inventory IDs

  2. Changes
    - Add inventory_id column to products table as foreign key to inventory
    - Populate inventory table with sample data for all branches
...
-- First, ensure the inventory table exists (in case migration didn't run)
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'inventory' AND policyname = 'Managers can access all inventory'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'inventory' AND policyname = 'Pharmacists can access their branch inventory'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'inventory' AND policyname = 'Pharmacists can update their branch inventory'
  ) THEN
...
-- Insert inventory data for all existing products across all branches
...
-- Add inventory_id column to products table
ALTER TABLE products ADD COLUMN IF NOT EXISTS inventory_id uuid REFERENCES inventory(id);

-- Update products with their primary inventory ID (using hcm-district1 as primary branch)
UPDATE products 
...
-- Create index for the new inventory_id column
